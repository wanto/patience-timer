class Game {
    ...
    player1_nextmove_button: HTMLButtonElement;
    player2_nextmove_button: HTMLButtonElement;
    player1_endturn_button: HTMLButtonElement;
    player2_endturn_button: HTMLButtonElement;

    player1_player_timer: HTMLDivElement;
    player2_player_timer: HTMLDivElement;

    start_game(): void {
        game.init();

        var input = (<HTMLInputElement>document.getElementById("playtime-input")).value.split("/");
    ...
    refresh_timer(): void {
        ...
        if(this.turn === "player1"){
            var diff:number = ((new Date().getTime() - this.turn_started.getTime()) / 1000) - this.bonus_time;
            var bonus:number; var timer:number;
    ...
    nextmove_handler_player1 () {
        if(this.turn === "starting")
            this.set_turn_player1 ();
        else
            this.next_move();
    }
    nextmove_handler_player2 () {
        if(this.turn === "starting")
    ...
    init() {
        this.player1_nextmove_button = <HTMLButtonElement>document.getElementById("player1-nextmove-button");
        this.player2_nextmove_button = <HTMLButtonElement>document.getElementById("player2-nextmove-button");
        this.player1_endturn_button = <HTMLButtonElement>document.getElementById("player1-endturn-button");
        this.player2_endturn_button = <HTMLButtonElement>document.getElementById("player2-endturn-button");

        this.player1_player_timer = <HTMLDivElement>document.getElementById("player1-player-timer");
        this.player2_player_timer = <HTMLDivElement>document.getElementById("player2-player-timer");
    }
}
